(() => {
    ...
    deserialize(data) {
      if (data) {
        try {
          ...
          Object.keys(storedPresets).map((name) => this.presets[name] = storedPresets[name]);
        } catch (e) {
          console.error(e);
        }
      }
  ...
  class Gui extends events_default {
    constructor(params) {
      ...
      this.listeners = {};
      this.config = params;
      this.presets = new presets_default();
    ...
    initPresets(params) {
      ...
      const initPrestList = () => {
        ...
        return folder.addInput(params, "currentPreset", {
          index: 0,
          label: "Current",
          options
      ...
      folder.addButton({title: "Save"}).on("click", () => {
        const presetName = prompt("Please enter the name for the current preset", this.config.currentPreset);
        if (!presetName) {
          return;
        }
        if (Object.keys(this.presets.get()).includes(presetName)) {
          if (!window.confirm("The preset with the same name already exists. Are you sure you want to overwrite it?")) {
            return;
          }
        }
      ...
      folder.addButton({title: "Delete"}).on("click", () => {
        if (window.confirm("Are you sure you want to delete the current preset?")) {
    ...
    initPattern(params) {
      ...
      folder.addInput(params, "patternScale", {
        label: "scale",
        min: 0.05,
        max: 4,
        step: 0.05
      }).on("change", this.handle("patternScale"));
      folder.addInput(params, "patternAngle", {
        label: "angle",
        min: -360,
        max: 360,
        step: 1
      ...
      folder.addInput(params, "makeTilable", {label: "Make Tilable"}).on("change", this.handle("makeTilable"));
      folder.addButton({title: "Load pattern..."}).on("click", () => {
        document.getElementById("patternFile").click();
      });
      folder.addButton({title: "Save Image..."}).on("click", () => this.trigger("click", "saveImage"));
    }
    initAnimation(params) {
      ...
      folder.addInput(params, "editAllKeyframes", {
        label: "Edit All Keyframes"
      }).on("change", this.handle("editAllKeyframes"));
      folder.addInput(params, "duration", {
        label: "Duration",
        min: 1,
        max: 600,
        step: 1
      }).on("change", this.handle("duration"));
      folder.addInput(params, "easing", {
        label: "Easing",
      ...
      prevButton = folder.addButton({title: "Previous keyframe", disabled: true}).on("click", () => {
        this.trigger("click", "prevKeyframe");
        this.updateAnimationGui();
      });
      nextButton = folder.addButton({title: "Next keyframe", disabled: true}).on("click", () => {
        this.trigger("click", "nextKeyframe");
        this.updateAnimationGui();
      });
      folder.addSeparator();
      newButton = folder.addButton({title: "New keyframe", disabled: true}).on("click", () => {
        this.trigger("click", "newKeyframe");
        this.updateAnimationGui();
      });
      deleteButton = folder.addButton({title: "Delete keyframe", disabled: true}).on("click", () => {
        this.trigger("click", "deleteKeyframe");
        this.updateAnimationGui();
      });
      resetButton = folder.addButton({title: "Clear keyframes", disabled: true}).on("click", () => {
        this.trigger("click", "resetKeyframes");
        this.updateAnimationGui();
      });
      folder.addSeparator();
      copyButton = folder.addButton({title: "Copy keyframe", disabled: true}).on("click", () => {
        this.trigger("click", "copyKeyframe");
        this.updateAnimationGui();
      });
      pasteButton = folder.addButton({title: "Paste keyframe", disabled: true}).on("click", () => {
      ...
      randomizeButton = folder.addButton({title: "Randomize", disabled: true}).on("click", () => this.trigger("click", "randomize"));
      folder.addSeparator();
      ...
      playButton = folder.addButton({title: "Play animation", disabled: true}).on("click", () => {
        this.trigger("click", "playAnimation");
        this.updateAnimationGui();
      });
      stopButton = folder.addButton({title: "Stop", hidden: true}).on("click", () => {
        this.trigger("click", "stopAnimation");
        this.updateAnimationGui();
      });
      renderButton = folder.addButton({title: "Export animation...", disabled: true}).on("click", () => {
  ...
  class Config {
    ...
    randomize() {
      ...
      this.symmetries = Math.floor(randomizeValue(this.symmetries, 1, 16, this.randomizeStrength));
    }
  }
